...
function stockDiagnosticRow(name,row){
  ...
  if(!valuationOk)problems.push('밸류에이션: PER·PBR 등 유효 값이 없습니다.');
  if(!flowOk)problems.push('수급: 외국인·기관 순매수 유효 값이 없습니다.');
...
function renderGlobalDiagnostics(payload){
  ...
  container.innerHTML=entries.map(([name,value])=>{
    ...
    const reason=diagnosticReason(info,name==='FnGuide'?`정상 캐시 재사용 ${Number(info.cached_stocks_reused||0)}개`:'세부 상태 없음');
    ...
    return `<details class="stock-diagnostic-item ${item.overall}" ${item.overall==='failed'?'open':''}>
      ...
      <div class="diagnostic-detail">
        <div class="diagnostic-source-grid">
          ...
          <div><span>PER·PBR</span>${diagnosticBadge(item.valuationOk?'ok':'unavailable',item.valuationOk?'정상':'자료 없음')}<small>PER ${compactNumber(item.valuation.per,'배')} · PBR ${compactNumber(item.valuation.pbr,'배')}</small></div>
          <div><span>외국인·기관 수급</span>${diagnosticBadge(item.flowOk?'ok':'unavailable',item.flowOk?'정상':'자료 없음')}<small>외국인 ${compactNumber(item.flow.foreign_net_buy_10d_krw,'원')} · 기관 ${compactNumber(item.flow.institution_net_buy_10d_krw,'원')}</small></div>
