...
def collect_one(name: str, code: str, previous: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        result["valuation"] = fallback.collect_valuation(session, code)
...
def main() -> None:
    ...
    print(f"Fast public market fallback: stocks={len(stocks)}, workers={MAX_WORKERS}")

    with ThreadPoolExecutor(max_workers=MAX_WORKERS, thread_name_prefix="market") as executor:
        futures = {
    ...
    payload.setdefault("source_status", {})["public_market_fallback"] = {
        "status": "ok" if any(counters.values()) else "failed",
    ...
    print(f"Fast public market fallback complete: {counters}; errors={len(errors)}")
